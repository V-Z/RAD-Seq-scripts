...
./rad_4_genotype_vcf_2_run.sh -w "raw.g.vcf" -u ".gz" -x ".join.vcf.gz" -f "${DATADIRB}" -c 7 -o "${DATADIRB}"_vcf -n "${DATADIRB}"_var -a "${REFB}" -j /packages/run/jdk-8/current/bin/java -m 63g -g /storage/pruhonice1-ibot/home/"${LOGNAME}"/bin/GenomeAnalysisTK.jar | tee "${DATADIRB}"_var_joining_genotype_vcf.log
# ./rad_4_genotype_vcf_2_run.sh -w "raw.g.vcf" -u ".gz" -x ".join.vcf.gz" -f "${DATADIRB}" -c 7 -o "${DATADIRB}"_vcf -n "${DATADIRB}"_all -a "${REFB}" -j /packages/run/jdk-8/current/bin/java -m 63g -g /storage/pruhonice1-ibot/home/"${LOGNAME}"/bin/GenomeAnalysisTK.jar -i | tee "${DATADIRB}"_all_joining_genotype_vcf.log
echo
