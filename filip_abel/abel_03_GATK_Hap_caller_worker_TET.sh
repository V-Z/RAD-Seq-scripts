...
# NB. to do so use command: n=1; for dir in A*; do mv "$dir" "$(printf "$dir.$n")";   n=$((n+1)); done
BAMDIR=~/aa/GATK/all_runs1234_oct_2017/tetraploids/A*.$TASK_ID
...
# do some work
# Run HaplotypeCaller on each TETRAPLOID sample to produce genomicVCF
cd $SCRATCH
