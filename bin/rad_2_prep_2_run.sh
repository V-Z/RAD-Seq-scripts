...
# See './rad_2_prep_2_run.sh -h' for help.
...
while getopts "hrvf:c:o:d:q:a:j:m:t:" INITARGS; do
	case "${INITARGS}" in
		h) # Help and exit
			...
			echo -e "\t-c\tNumber of CPU threads to use for parallel operations (parameter \"-j\" of GNU Parallel). If not provided, default is 4."
			...
			echo -e "\t-j\tOptional path to custom Java binary (default is output of \"command -v java\")."
		...
		o) # Output directory for trimmed sequences
			if [ -d "${OPTARG}" ]; then
				...
				echo
					...
					echo "Output directory for trimmed sequences ${TRIMDIR} doesn't exist - creating '2_trimmed'."
					TRIMDIR='2_trimmed'
		...
		d) # Output directory for deduplicated sequences
			if [ -d "${OPTARG}" ]; then
				...
				echo
					...
					echo "Output directory for deduplicated sequences ${DEDUPDIR} doesn't exist - creating '3_dedup'."
					DEDUPDIR='3_dedup'
		...
		q) # Output directory for quality reports
			if [ -d "${OPTARG}" ]; then
				...
				echo
					...
					echo "Output directory for quality reports ${QUALDIR} doesn't exist - creating '4_qual_rep'."
					QUALDIR='4_qual_rep'
...
# Initialize files with statistics
...
for FASTQ1 in "${FQDIR}"/*/*.R1.f*q*; do
	...
	mkdir -p "${TRIMDIR}"/"${FASTQBASE}" "${DEDUPDIR}"/"${FASTQBASE}" "${QUALDIR}"/"${FASTQBASE}" || operationfailed
	echo

	# Pre-processing of the reads
	echo "Trimming"
	# Trimming and adaptor removal with Trimmomatic
	${JAVA} -Xmx"$((MEM*NCPU))"g -jar "${TRIMMOMATIC}" PE -threads "$((NCPU-1))" -phred33 "${FASTQ1}" "${FASTQ2}" "${TRIMDIR}"/"${FASTQBASE}"/"${TRM1}" "${TRIMDIR}"/"${FASTQBASE}"/"${UNP1}" "${TRIMDIR}"/"${FASTQBASE}"/"${TRM2}" "${TRIMDIR}"/"${FASTQBASE}"/"${UNP2}" ILLUMINACLIP:"${ADAPTOR}":2:30:10 SLIDINGWINDOW:5:20 LEADING:20 TRAILING:20 MINLEN:50 || operationfailed
	echo
	...
	{ printf '%s\t' "${FASTQ}" # Print sample name
		...
		echo "$(($(wc -l "${TRIMDIR}"/"${FASTQBASE}"/"${TRM1}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of trimmed R1 reads (N of lines /4)
		echo "$(($(wc -l "${TRIMDIR}"/"${FASTQBASE}"/"${TRM2}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of trimmed R2 reads (N of lines /4)
		echo "$(($(wc -l "${TRIMDIR}"/"${FASTQBASE}"/"${UNP1}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of unpaired R1 reads (N of lines /4)
		echo "$(($(wc -l "${TRIMDIR}"/"${FASTQBASE}"/"${UNP2}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of unpaired R2 reads (N of lines /4)
		printf '\n'
		} >> "${TRIMDIR}"/report_trimming.tsv || operationfailed
	echo
	...
	clumpify.sh in="${TRIMDIR}"/"${FASTQBASE}"/"${TRM1}" in2="${TRIMDIR}"/"${FASTQBASE}"/"${TRM2}" out="${DEDUPDIR}"/"${FASTQBASE}"/"${NODUP1}" out2="${DEDUPDIR}"/"${FASTQBASE}"/"${NODUP2}" dedupe optical spany adjacent -Xmx"$((MEM*NCPU))"g || operationfailed
	echo
	...
	{ printf '%s\t' "${FASTQ}" # Print sample name
		...
		echo "$(($(wc -l "${TRIMDIR}"/"${FASTQBASE}"/"${TRM1}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of trimmed R1 reads (N of lines /4)
		echo "$(($(wc -l "${TRIMDIR}"/"${FASTQBASE}"/"${TRM2}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of trimmed R2 reads (N of lines /4)
		echo "$(($(wc -l "${DEDUPDIR}"/"${FASTQBASE}"/"${NODUP1}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of deduplicated R1 reads (N of lines /4)
		echo "$(($(wc -l "${DEDUPDIR}"/"${FASTQBASE}"/"${NODUP2}" | cut -f 1 -d ' ')/4))" | xargs printf '%s\t%s' # No of deduplicated R2 reads (N of lines /4)
		printf '\n'
		} >> "${DEDUPDIR}"/report_filtering.tsv || operationfailed
	echo
	...
	fastqc -o "${QUALDIR}"/"${FASTQBASE}" -t "${NCPU}" "${FASTQ1}" "${FASTQ2}" "${TRIMDIR}"/"${FASTQBASE}"/"${TRM1}" "${TRIMDIR}"/"${FASTQBASE}"/"${TRM2}" "${DEDUPDIR}"/"${FASTQBASE}"/"${NODUP1}" "${DEDUPDIR}"/"${FASTQBASE}"/"${NODUP2}" || operationfailed
	echo
	done
